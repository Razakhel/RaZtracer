...
Matrix<T, W, H>::Matrix(const Matrix<TI, WI, HI>& mat) : Matrix(mat.getWidth(), mat.getHeight()) {
...
template <typename T, unsigned int W, unsigned int H>
Matrix<T, W, H>::Matrix(const std::initializer_list<const std::initializer_list<T>>& list)
